...
- The module is not yet registered in `{project-root}/_bmad/bmadconfig.yaml`
...
2. Check if `{project-root}/_bmad/bmadconfig.yaml` exists — if a section matching the module's code is already present, inform the user this is an update (reconfiguration)
...
Only collect if no core keys exist yet in `bmadconfig.yaml` or `config.user.yaml`:
...
- `output_folder` (default: `{project-root}/_bmad-output`) — written to `bmadconfig.yaml` at root, shared across all modules
...
```bash
python3 ./scripts/merge-config.py --config-path "{project-root}/_bmad/bmadconfig.yaml" --user-config-path "{project-root}/_bmad/config.user.yaml" --module-yaml ./assets/module.yaml --answers {temp-file}
...
Use the script JSON output to display what was written — config values set (written to `bmadconfig.yaml` at root for core, module section for module values), user settings written to `config.user.yaml` (`user_keys` in result), help entries added, fresh install vs update.
