...
Workflows read config from `{project-root}/_bmad/bmadconfig.yaml` and `config.user.yaml`.
...
```
Load config from {project-root}/_bmad/bmadconfig.yaml ({module-code} section) and config.user.yaml.
...
```
Load config from {project-root}/_bmad/bmadconfig.yaml and config.user.yaml if available.
