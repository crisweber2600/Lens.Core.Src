...
{if-module}
Load available config from `{project-root}/_bmad/bmadconfig.yaml` and `{project-root}/_bmad/config.user.yaml` (root level and `{module-code}` section).
{/if-module}
{if-standalone}
Load available config from `{project-root}/_bmad/bmadconfig.yaml` and `{project-root}/_bmad/config.user.yaml` if present.
{/if-standalone}
