---
model: Claude Sonnet 4.6 (copilot)
description: 'Run bmad-create-architecture with Lens feature, repo, and governance context'
---

# lens-work.bmad-create-architecture (Stub)

> **This is a stub.** Load and execute the full prompt from the release module.
> All `lens.core/_bmad/` paths in the full prompt are relative to `lens.core/` — do NOT resolve paths against the user's main project repo.

```
Read and follow all instructions in: lens.core/_bmad/lens-work/prompts/lens-work.bmad-create-architecture.prompt.md
```
