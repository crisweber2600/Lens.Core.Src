...
if [[ "$DRY_RUN" != true ]]; then
  mkdir -p "${PROJECT_ROOT}/docs/lens-work/initiatives"
  mkdir -p "${PROJECT_ROOT}/docs/lens-work/personal"
  log_ok "Output directory structure verified"
else
  log_info "[DRY-RUN] Would create docs/lens-work/ directories"
fi

# -- 4b. Write governance-setup.yaml ----------------------------------------
GOVERNANCE_SETUP_PATH="${PROJECT_ROOT}/docs/lens-work/governance-setup.yaml"
if [[ "$DRY_RUN" != true ]]; then
