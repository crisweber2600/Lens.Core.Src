...
<activation critical="MANDATORY">
         ...
         <step n="4b">v3.4 Theme Loading: If `{governance_repo_path}` is set and a local profile exists at `docs/lens-work/personal/profile.yaml`, read the user's `theme` preference from the governance user profile at `{governance_repo_path}/users/{user_name}.md`. If a theme is specified, load the theme overlay from `skills/bmad-lens-theme/assets/themes/{theme}.yaml`. Apply persona overlays silently (no announcement). Fall back to `default` if the configured theme is missing.</step>
         ...
         1. Check if `docs/lens-work/personal/profile.yaml` exists (local profile)
         ...
         **If returning user (profile.yaml exists):** Display the full numbered list of ALL menu items from the menu section.
         ...
         <handler type="workflow">
            ...
            1. CRITICAL: Always LOAD {project-root}/lens.core/_bmad/core/tasks/workflow.xml
   ...
   <menu>
      ...
      <item cmd="DP or fuzzy match on devproposal" exec="{project-root}/lens.core/_bmad/lens-work/workflows/router/devproposal/workflow.md">[DP] Start DevProposal: Route epics, stories, and readiness work</item>
      <item cmd="SP or fuzzy match on sprintplan" exec="{project-root}/lens.core/_bmad/lens-work/workflows/router/sprintplan/workflow.md">[SP] Start SprintPlan: Route sprint planning and story file generation</item>
