...
Run:
    ...
    uv run --with pytest pytest {project-root}/lens.core/_bmad/lens-work/skills/bmad-lens-techplan/scripts/tests/test-techplan-ops.py -q
"""
...
#   test file: {project-root}/lens.core/_bmad/lens-work/skills/bmad-lens-techplan/scripts/tests/test-techplan-ops.py
#   parents[0] = tests/
#   parents[1] = scripts/
#   parents[2] = bmad-lens-techplan/  ← SKILL_ROOT
#   parents[3] = skills/
#   parents[4] = lens-work/           ← MODULE_ROOT (via SKILL_ROOT.parents[1])
#   parents[5] = {project-root}/lens.core/_bmad/
...
def test_stub_preflight_then_release_prompt():
    ...
    preflight = "uv run {project-root}/lens.core/_bmad/lens-work/skills/bmad-lens-preflight/scripts/light-preflight.py"
...
def test_release_prompt_exists():
    """Release prompt must exist under {project-root}/lens.core/_bmad/lens-work/prompts/."""
    assert RELEASE_PROMPT.exists(), (
        f"Missing release prompt: {RELEASE_PROMPT}\n"
        "Create {project-root}/lens.core/_bmad/lens-work/prompts/lens-techplan.prompt.md (TK-2.3)"
    )
...
def test_release_prompt_routes_to_conductor_skill():
    ...
    assert "bmad-lens-techplan/SKILL.md" in text, (
        "Release prompt must reference {project-root}/lens.core/_bmad/lens-work/skills/bmad-lens-techplan/SKILL.md"
    )
...
def test_conductor_skill_exists():
    ...
    assert CONDUCTOR_SKILL.exists(), (
        ...
        "Create {project-root}/lens.core/_bmad/lens-work/skills/bmad-lens-techplan/SKILL.md (TK-2.3)"
    )
