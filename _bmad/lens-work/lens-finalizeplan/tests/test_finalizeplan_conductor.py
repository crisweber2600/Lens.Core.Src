...
class TestFinalizePlanContract:
    ...
    def test_governance_writes_are_only_through_allowed_boundaries(self):
        ...
        assert "bmad-lens-git-orchestration" in content
        assert "bmad-lens-feature-yaml" in content
    ...
    def test_step_three_delegates_bundle_in_required_order(self):
        ...
        expected = [
            "bmad-lens-bmad-skill --skill bmad-create-epics-and-stories",
            "bmad-lens-bmad-skill --skill bmad-check-implementation-readiness",
            "bmad-lens-bmad-skill --skill bmad-sprint-planning",
            "bmad-lens-bmad-skill --skill bmad-create-story",
        ]
...
class TestFinalizePlanDiscovery:
    def test_prompt_chain_uses_light_preflight_then_redirects(self):
        ...
        assert "bmad-lens-finalizeplan/SKILL.md" in release_prompt
    ...
    def test_module_help_has_user_facing_finalizeplan_entry(self):
        ...
        matches = [row for row in rows if row["skill"] == "bmad-lens-finalizeplan"]
    ...
    def test_module_help_review_args_include_new_phases(self):
        ...
        matches = [row for row in rows if row["skill"] == "bmad-lens-adversarial-review"]
        assert len(matches) == 1
