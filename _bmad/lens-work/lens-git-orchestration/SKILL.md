...
I create and manage branches for Lens features — I enforce the control repo 2-branch model (`featureId` + `featureId-plan`), prepare the target repo working branch for Dev, and commit planning artifacts. I do not modify `feature.yaml` (that is `bmad-lens-feature-yaml`'s job) and I do not query state (use `bmad-lens-git-state`).
...
4. Invoke the CLI-backed publish step, either through this skill or directly via `uv run {project-root}/lens.core/_bmad/lens-work/lens-git-orchestration/scripts/git-orchestration-ops.py publish-to-governance --governance-repo {governance_repo} --control-repo {control_repo} --feature-id {feature_id} --phase {phase}`.
