...
- Run the phase-complete adversarial review through `lens-adversarial-review --phase preplan --source phase-complete` in party mode.
...
```bash
uv run {project-root}/lens.core/_bmad/lens-work/skills/lens-validate-phase-artifacts/scripts/validate-phase-artifacts.py \
   --phase preplan \
...
|---|---|---|
| Feature state | `lens-feature-yaml` | Resolve feature context on activation; update phase only after the adversarial review passes. |
| Cross-feature context | `lens-init-feature fetch-context` | Load related summaries and dependency docs before authoring decisions. |
| Constitution | `lens-constitution` | Load governed constraints; shared constitution handling owns partial hierarchy behavior. |
| Batch intake and resume | `lens-batch --target preplan` | Pass 1 stops after intake with no lifecycle artifacts; pass 2 resumes from `batch_resume_context`. |
...
| Divergent brainstorm | `lens-bmad-skill` -> `bmad-brainstorming` | User-selected route that must complete before downstream authoring. |
| Structured brainstorm | `lens-bmad-skill` -> `bmad-cis` | User-selected route that must complete before downstream authoring. |
| Research | `lens-bmad-skill` -> `bmad-domain-research`, `bmad-market-research`, or `bmad-technical-research` | Available only after `brainstorm.md` exists. |
| Product brief | `lens-bmad-skill` -> `bmad-product-brief` | Available only after `brainstorm.md` exists. |
| Phase gate | `lens-adversarial-review --phase preplan --source phase-complete` | Party-mode review; fail blocks phase update, pass and pass-with-warnings allow it. |
| Control artifact staging | `lens-git-orchestration` | Stage or commit control-repo artifacts when the lifecycle flow requires it; never publish governance from PrePlan. |
