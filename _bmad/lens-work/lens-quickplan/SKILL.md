...
QuickPlan is an internal-only Lens skill used by ExpressPlan. It runs a compact three-phase planning pipeline that produces business, technical, and sprint planning artifacts in the feature docs path. It is called via `lens-bmad-skill` only and has no public prompt stub.
