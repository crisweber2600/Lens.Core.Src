...
function Invoke-GitHubPrCreate {
    ...
    # Set environment variable for gh auth
    $env:GH_TOKEN = $Pat
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            $PrUrl = $Result | Select-String -Pattern 'https://.*' | Select-Object -First 1
            return $PrUrl.ToString().Trim()
        } else {
            Write-Host "  ❌ PR creation failed: $Result" -ForegroundColor Red
            return $null
        }
    } catch {
        Write-Host "  ❌ PR creation error: $($_.Exception.Message)" -ForegroundColor Red
        return $null
    } finally {
...
if ($pat -and $remoteInfo.Platform -eq 'github' -and $CreatePR -and -not $UrlOnly) {
    ...
    if ($CreatedPrUrl) {
        Write-Host "✅ PR created: $CreatedPrUrl" -ForegroundColor Green
        $prCreated = $true
    } else {
        Write-Host "⚠️  PR creation failed. Manual URL: $prUrl" -ForegroundColor Yellow
    }
    Write-Host ""
}
