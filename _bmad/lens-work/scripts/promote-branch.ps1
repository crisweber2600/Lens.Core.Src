...
function Invoke-GitHubPrCreate {
    ...
    if (-not $GhPath) {
        Write-Host "  [WARN]  GitHub CLI (gh) not found. Install from: https://cli.github.com" -ForegroundColor Yellow
        return $null
    }
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "  [ERROR] PR creation failed: $Result" -ForegroundColor Red
            return $null
        }
    } catch {
        Write-Host "  [ERROR] PR creation error: $($_.Exception.Message)" -ForegroundColor Red
        return $null
    } finally {
