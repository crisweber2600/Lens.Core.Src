...
7. After quick-dev delegation returns, run this conductor completion gate before responding to the user. This gate is mandatory even if the delegate claims the work is complete:
   ...
   - For automatic completion requests, switch the control repo to the shared `dev` branch before the completion handoff and rely on `complete-ops.py` to create and merge the `dev` → `main` PR.
