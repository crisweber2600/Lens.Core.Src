...
5. Validate the predecessor phase gate:
   ...
   - If phase wording is active `techplan` or active `expressplan`, continue only when `validate-phase-artifacts.py --phase {phase} --contract review-ready --json` passes and the user is resuming a phase-complete handoff.
