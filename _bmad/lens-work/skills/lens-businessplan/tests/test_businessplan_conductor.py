...
# Test file is at: {repo_root}/_bmad/lens-work/skills/lens-businessplan/tests/test_businessplan_conductor.py
# So we need 6 parents to get to repo root
REPO_ROOT = Path(__file__).parent.parent.parent.parent.parent.parent
SKILL_PATH = REPO_ROOT / "_bmad/lens-work/skills/lens-businessplan/SKILL.md"
...
class TestWrapperEquivalence:
    ...
    def test_batch_pass_1_delegates_to_bmad_lens_batch(self):
        """Batch pass 1 must delegate to lens-batch with --target businessplan."""
        content = SKILL_PATH.read_text(encoding="utf-8")
        
        # Should delegate to lens-batch
        assert "lens-batch" in content, "Missing lens-batch delegation"
    ...
    def test_review_ready_fast_path_validates_then_reviews(self):
        ...
        assert "lens-adversarial-review" in content, "Missing adversarial review delegation"
    ...
    def test_prd_route_uses_bmad_lens_bmad_skill(self):
        """PRD authoring must route through lens-bmad-skill, not direct skill invocation."""
        content = SKILL_PATH.read_text(encoding="utf-8")
        
        # Should delegate through lens-bmad-skill
        assert "lens-bmad-skill" in content, "Missing lens-bmad-skill wrapper"
        ...
        # PRD route should use lens-bmad-skill
        prd_pattern = r"`prd`.*?lens-bmad-skill.*?bmad-create-prd"
        assert re.search(prd_pattern, delegation_section, re.DOTALL), \
            "PRD route should delegate through lens-bmad-skill"

    def test_ux_route_uses_bmad_lens_bmad_skill(self):
        """UX authoring must route through lens-bmad-skill, not direct skill invocation."""
        content = SKILL_PATH.read_text(encoding="utf-8")
        
        # Should delegate through lens-bmad-skill for UX
        assert "lens-bmad-skill" in content, "Missing lens-bmad-skill wrapper"
        ...
        # UX route should use lens-bmad-skill
        ux_pattern = r"`ux-design`.*?lens-bmad-skill.*?bmad-create-ux-design"
        assert re.search(ux_pattern, delegation_section, re.DOTALL), \
            "UX route should delegate through lens-bmad-skill"
...
class TestDiscoverySurface:
    ...
    def test_module_help_has_businessplan_entry(self):
        """module-help.csv must have lens-businessplan entry with correct metadata."""
        ...
        for row in rows:
            if row["skill"] == "lens-businessplan":
                businessplan_row = row
                break
        
        assert businessplan_row is not None, "Missing lens-businessplan entry in module-help.csv"
    ...
    def test_module_help_businessplan_after_preplan(self):
        """businessplan entry must appear after lens-preplan:plan in module-help.csv."""
        with open(MODULE_HELP, encoding="utf-8") as f:
            reader = csv.DictReader(f)
            rows = list(reader)
        ...
        for idx, row in enumerate(rows):
            if row["skill"] == "lens-preplan" and row["action"] == "plan":
                preplan_idx = idx
            if row["skill"] == "lens-businessplan":
                businessplan_idx = idx
        
        assert preplan_idx is not None, "Missing lens-preplan:plan entry"
        assert businessplan_idx is not None, "Missing lens-businessplan entry"
    ...
    def test_module_help_businessplan_before_techplan(self):
        """businessplan entry must appear before lens-techplan:plan in module-help.csv."""
        with open(MODULE_HELP, encoding="utf-8") as f:
            reader = csv.DictReader(f)
            rows = list(reader)
        ...
        for idx, row in enumerate(rows):
            if row["skill"] == "lens-businessplan":
                businessplan_idx = idx
            if row["skill"] == "lens-techplan" and row["action"] == "plan":
                techplan_idx = idx
        
        assert businessplan_idx is not None, "Missing lens-businessplan entry"
        assert techplan_idx is not None, "Missing lens-techplan:plan entry"
    ...
    def test_agent_routes_through_help_and_next(self):
        ...
        assert "lens-help" in content.lower() or "lens-help" in content, \
            "Agent must reference help skill"
        assert "lens-next" in content.lower() or "lens-next" in content, \
...
class TestPromptChain:
    ...
    def test_release_prompt_loads_skill(self):
        ...
        assert "lens-businessplan/SKILL.md" in content, \
    ...
    def test_release_prompt_delegates_to_skill(self):
        ...
        forbidden_inline_patterns = [
            ...
            r"\binvoke\s+.*?\blens-bmad-skill\b",
