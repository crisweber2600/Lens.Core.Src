...
EXPRESS_SKILL = REPO_ROOT / "_bmad/lens-work/skills/lens-expressplan/SKILL.md"
QUICKPLAN_SKILL = REPO_ROOT / "_bmad/lens-work/skills/lens-quickplan/SKILL.md"
WRAPPER_SKILL = REPO_ROOT / "_bmad/lens-work/skills/lens-bmad-skill/SKILL.md"
...
class TestExpressPlanContract:
    def test_express_gate_and_constitution_gate_precede_delegation(self):
        ...
        delegation_index = activation.find("lens-bmad-skill")
...
class TestQuickPlanInternalOnly:
    def test_quickplan_declares_internal_only_and_outputs(self):
        ...
        assert "called via `lens-bmad-skill` only" in content
    ...
    def test_wrapper_table_registers_quickplan_internal_target(self):
        ...
        assert "lens-quickplan" in content
    ...
    def test_wrapper_registry_registers_quickplan_internal_target(self):
        ...
        entry = data["skills"]["lens-quickplan"]
        ...
        assert entry["entryPath"] == "skills/lens-quickplan/SKILL.md"
...
class TestExpressPlanDiscovery:
    ...
    def test_module_help_has_expressplan_but_no_quickplan_entries(self):
        ...
        express_rows = [row for row in rows if row["skill"] == "lens-expressplan"]
        quickplan_rows = [row for row in rows if row["skill"] == "lens-quickplan"]

        assert len(express_rows) == 1
        assert express_rows[0]["display-name"] == "expressplan"
        assert quickplan_rows == []

        batch_rows = [row for row in rows if row["skill"] == "lens-batch"]
        assert batch_rows
