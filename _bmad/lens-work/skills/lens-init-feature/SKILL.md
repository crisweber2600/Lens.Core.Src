---
name: lens-init-feature
description: Feature initialization orchestrator — creates topology-aware control branches, feature.yaml, PR guidance, and feature-index entry. Use when starting a new feature from scratch.
---
...
- **Progressive disclosure** — prompt for name, domain, service; derive canonical featureId; ask the user to choose a track explicitly; proceed without a separate featureId confirmation prompt
...
- **Explicit repo handoff** — init-feature establishes the feature and its docs path, but target repository provisioning is a separate step owned by `lens-target-repo`
...
| Skill | Relationship |
|-------|-------------|
| `lens-onboard` | ~~Prerequisite~~ — **deprecated**; governance_repo is now configured via `lens-new-domain` and `lens-new-service` |
| `lens-feature-yaml` | Delegate — init-feature creates the initial feature.yaml; feature-yaml manages subsequent lifecycle |
| `lens-target-repo` | Follow-up repo orchestration when the feature needs a new target implementation repository |
| `lens-next` | Lifecycle router — resolves the correct post-init follow-up command for the selected track |
| `lens-quickplan` | Optional planning wrapper — available for supported tracks, but not the universal first step after init-feature |
| `lens-theme` | Loaded on activation for persona overlay |
...
Feature init also returns branch and activation follow-up commands when callers pass a separate `--control-repo`; omitted or governance-equivalent control repo values intentionally skip those commands so the governance repo stays on `main`. The git commands route through `lens-git-orchestration create-feature-branches` with the resolved `control_topology`. The activation command then routes through `lens-switch switch` so the control repo lands on the topology-correct branch and local Lens context is updated.
