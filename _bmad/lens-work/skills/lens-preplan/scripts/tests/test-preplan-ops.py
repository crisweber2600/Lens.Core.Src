...
def test_both_brainstorm_routes_and_brainstorm_first_gate():
    ...
    assert "`lens-bmad-skill`" in authoring
...
def test_batch_pass_one_and_pass_two_contract():
    ...
    assert "lens-batch --target preplan" in text
...
def test_review_ready_delegates_to_shared_validator():
    ...
    assert "$PYTHON _bmad/lens-work/scripts/validate-phase-artifacts.py" in text
...
def test_phase_gate_fail_and_pass_contract():
    ...
    assert "lens-adversarial-review --phase preplan --source phase-complete" in completion
    ...
    assert "lens-feature-yaml" in completion
    assert_order(completion, "lens-adversarial-review", "lens-feature-yaml")
...
def test_next_handoff_and_fetch_context_contracts():
    ...
    assert "lens-init-feature fetch-context" in text
    assert_order(text, "lens-init-feature fetch-context", "lens-constitution")
...
def test_integration_table_contains_required_delegations():
    ...
    for token in (
        ...
        "bmad-product-brief",
        "lens-adversarial-review --phase preplan --source phase-complete",
...
def test_module_help_and_shell_discovery_are_aligned_without_duplicates():
    ...
    assert module_help.count("lens-preplan,preplan") == 1
