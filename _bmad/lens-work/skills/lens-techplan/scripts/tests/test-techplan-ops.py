...
#   test file: _bmad/lens-work/skills/lens-techplan/scripts/tests/test-techplan-ops.py
#   parents[0] = tests/
#   parents[1] = scripts/
#   parents[2] = lens-techplan/  ← SKILL_ROOT
#   parents[3] = skills/
#   parents[4] = lens-work/           ← MODULE_ROOT (via SKILL_ROOT.parents[1])
#   parents[5] = _bmad/
#   parents[6] = lens.core.src/       ← REPO_ROOT (via MODULE_ROOT.parents[1])
# ---------------------------------------------------------------------------
SKILL_ROOT = Path(__file__).resolve().parents[2]       # lens-techplan/
...
def test_release_prompt_routes_to_conductor_skill():
    ...
    assert "lens-techplan/SKILL.md" in text, (
        "Release prompt must reference _bmad/lens-work/skills/lens-techplan/SKILL.md"
    )
...
def test_conductor_skill_exists():
    """Conductor SKILL.md must exist under skills/lens-techplan/."""
    assert CONDUCTOR_SKILL.exists(), (
        f"Missing conductor skill: {CONDUCTOR_SKILL}\n"
        "Create _bmad/lens-work/skills/lens-techplan/SKILL.md (TK-2.3)"
    )
...
def test_conductor_skill_is_conductor_only():
    ...
    assert "lens-bmad-skill" in text, (
        "Conductor skill must reference lens-bmad-skill for BMAD wrapper delegation"
    )
...
def test_publish_gate_runs_before_architecture_authoring():
    ...
    assert "publish-to-governance" in text, (
        "Conductor skill must invoke publish-to-governance hook (lens-git-orchestration)"
    )
    ...
    delegate_idx = text.index("lens-bmad-skill --skill bmad-create-architecture")
...
def test_adversarial_review_gate_enforced():
    ...
    text = read_text(CONDUCTOR_SKILL)
    assert "lens-adversarial-review" in text, (
        "Conductor skill must reference lens-adversarial-review gate"
    )
...
def test_constitution_loader_activated():
    """Conductor must load domain constitution via lens-constitution."""
    text = read_text(CONDUCTOR_SKILL)
    assert "lens-constitution" in text, (
        "Conductor skill must invoke lens-constitution loader"
    )
    assert "[TK-3.1]" not in text
...
def test_pr1_publish_gate_stops_before_authoring():
    ...
    assert text.index("publish-to-governance") < text.index("lens-bmad-skill --skill"), (
...
def test_pr3_adversarial_review_gate_blocks_if_absent():
    ...
    review_idx = text.index("lens-adversarial-review")
    delegate_idx = text.index("lens-bmad-skill --skill bmad-create-architecture")
...
def test_pr5_conductor_delegates_only_no_inline_authoring():
    ...
    assert "lens-bmad-skill --skill bmad-create-architecture" in text
