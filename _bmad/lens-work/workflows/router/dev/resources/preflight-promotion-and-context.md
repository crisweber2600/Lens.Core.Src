...
```yaml
# v3.4: 2-branch topology — validate via feature.yaml phase state, skip audience checks
if session.feature_yaml_context != null and session.feature_yaml_context.enabled == true:

  # In 2-branch topology, sprintplan completion is tracked in feature.yaml, not audience branches
  if initiative.phase_status["sprintplan"] == "complete":
    output: "✅ Previous phase (sprintplan) complete (2-branch topology)"
  elif initiative.phase_status["sprintplan"] == "pr_pending":
    pr_url = initiative.phase_status.sprintplan_pr_url || "(no PR URL recorded)"
    output: |
      ⚠️  Previous phase (sprintplan) PR not yet merged
      ├── Status: pr_pending
      ├── PR: ${pr_url}
      └── You may continue, but phase artifacts may not be finalized
    ...
    error: |
      ❌ SprintPlan phase not complete
      └── Action: Complete /sprintplan first

  # No audience promotion needed in 2-branch topology — skip directly to constitutional context

else:
  # Legacy topology — audience-based validation follows
