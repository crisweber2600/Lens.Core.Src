...
Runs inline governance checks at every workflow step. Replaces the Scribe/Cornelius agent from lens-work. Constitution checks are skills, not separate workflows — they execute as part of every workflow step automatically.
...
Default mode is `advisory`. Configurable per initiative.

## Trigger Conditions

- **Automatic** at every workflow step (via background_triggers)
- Not user-invoked — constitution checks are invisible skills
- Results surface only when violations are found
