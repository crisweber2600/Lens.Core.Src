...
│   │   ├── lens-work-checklist/
│   │   ├── lens-work-constitution/
│   │   ├── lens-work-git-orchestration/
│   │   ├── lens-work-git-state/
│   │   └── lens-work-sensing/
...
│       ├── docs/                             # 21 embedded reference docs
...
│       ├── module-help.csv
│       └── module.yaml
├── docs/                                     # Parent-level comprehensive scan output
│   ├── .archive/
...
**Why it matters:** The promotion pipeline overlays these files onto generated adapter output. They are source-controlled product assets, not scratch files.

### `_bmad/`

**Purpose:** Source root for the module and bridge configuration.

**Contains:** `_bmad/config.yaml` plus the `lens-work` module directory.
...
**Entry Points:** `module.yaml`, `lifecycle.yaml`, `scripts/install.py`, `scripts/preflight.py`, `agents/lens.agent.md`, and `_module-installer/installer.js`.
...
**Purpose:** Parent-level source-project documentation set created by this workflow.

**Contains:** Contributor-facing overview, architecture, source tree, component inventory, development and deployment guidance, contribution guidance, and the scan state file.
...
- `_bmad/config.yaml`: bridge configuration used by Lens wrapper flows.
